...
namespace mpba\DB2\Database\Query\Grammars;
...
class DB2Grammar extends Grammar
{
    ...
    protected function wrapValue($value)
    {
        ...
        // Specific work around for migration status
        if ($value === 'migration' or $value === 'migrations' or $value === 'batch') {
            if ($value === 'migrations') {
                $value = 'MIGRATIONS';
            }
    ...
    public function compileSelect(Builder $query)
    {
        ...
        $value = $this->concatenate($components);
